...
function runMigrations(db: SqliteDatabase): void {
  ...
  ensureColumn(db, "worktrees", "git_dir", "TEXT");
  // Fold the active AI provider/model/agent version into the tour cache key so a
  // tour generated by one configuration is not served after the user switches.
  ensureColumn(db, "ai_tours", "settings_hash", "TEXT NOT NULL DEFAULT ''");
}
